...
class Asset extends CI_Controller{
    ...
    function index($offset=0,$order_column='kode_asset',$order_type='asc'){
        ...
        if($this->uri->segment(3)=="delete_success")
            ...
            $data['message']='';
            $this->template->display('asset/index',$data);
    }
    ...
    function tambah(){
        ...
        if($this->form_validation->run()==true){//jika validasi dijalankan dan benar
            ...
            if($cek->num_rows()>0){ // jika kode_asset sudah ada, maka tampilkan pesan
                $data['message']="<div class='alert alert-danger'>Kode Asset sudah ada</div>";
                $this->template->display('asset/tambah',$data);
    ...
    function edit($id){
	    ...
	    $config['max_height']  = '1024';
            ...
            $this->template->display('asset/edit',$data);
        }else{
            $data['message']="";
            $data['asset']=$this->m_asset->cek($id)->row_array();
            $this->template->display('asset/edit',$data);
    ...
    function cari(){
        ...
        if($cek->num_rows()>0){
            ...
            $this->template->display('asset/cari',$data);
        }else{
    ...
    function _set_rules(){
        $this->form_validation->set_rules('kode_asset','Kode Asset','required|max_length[5]');
