<?php
class Pengembalian1 extends CI_Controller{
    #code
    
    function __construct(){
        parent::__construct();
        $this->load->library(array('template1','form_validation'));
    ...
    function index(){
        $data['title']="Pengembalian Asset";
        $data['tanggal']=date('Y-m-d');
        $this->template1->display('pengembalian/index',$data);
    }
    ...
    function tampil(){
        $no=$_GET['no'];
        $data['asset']=$this->m_pengembalian->tampilAsset($no)->result();
        $transaksi=$this->m_pengembalian->cariTransaksi($no)->row_array();
        
        $this->load->view('pengembalian/tampilasset',$data);
    }
