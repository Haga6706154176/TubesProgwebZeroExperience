<?php
class Anggota2 extends CI_Controller{
    private $limit=20;
    
    function __construct(){
        parent::__construct();
        $this->load->library(array('template2','pagination','form_validation','upload'));
    ...
    function index($offset=0,$order_column='nis',$order_type='asc'){
        ...
        if($this->uri->segment(3)=="delete_success")
            ...
            $data['message']='';
            $this->template2->display('anggota/index',$data);
    }
    ...
    function edit($id){
	    ...
	    $config['max_height']  = '1024';
            ...
            $this->template2->display('anggota/edit',$data);
        }else{
            $data['anggota']=$this->m_anggota->cek($id)->row_array();
            $data['message']="";
            $this->template2->display('anggota/edit',$data);
    ...
    function tambah(){
        ...
        if($this->form_validation->run()==true){
            ...
            if($cek->num_rows()>0){
                ...
                $this->template2->display('anggota/tambah',$data);
            }else{
                //setting konfiguras upload image
                $config['upload_path'] = './assets/img/anggota/';
		$config['allowed_types'] = 'gif|jpg|png';
    ...
    function cari(){
        ...
        if($cek->num_rows()>0){
            ...
            $this->template2->display('anggota/cari',$data);
        }else{
